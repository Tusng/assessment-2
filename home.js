...
const canWeDeliver = (zipCode) => {
    if (deliveryAreaZipCodes.includes(zipCode)) {
        return `The zip code ${zipCode} is eligible for delivery`;
    } else {
        return `Our sincere apologies, we can't deliver to ${zipCode}`;
    }
};
...
function canWeDeliverTwo (zipCode) {
    for (currZipCode of deliveryAreaZipCodes) {
        if (currZipCode === zipCode) {
            return `The zip code ${zipCode} is eligible for delivery`; 
        }
    }
    return `Our sincere apologies, we can't deliver to ${zipCode}`;
...
//CODE HERE
for (currObject of deals) {
    currObject.title = currObject['title'].replace("15", "10");
}
...
//CODE HERE
for (currObject of deals) {
    currObject.desc = currObject['desc'].replace("March", "April");
}
console.log(deals);