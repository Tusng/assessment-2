...
const pizza = {
    ...
    popularity: 1,
    rating: 4.8,
    tags: ['popular', 'common', 'delicous', 'gluten-free']
};
...
//CODE HERE
console.log(`Second tag: ${pizza.tags[2]}`);
...
const foodArr = [
    {
        ...
        popularity: 1,
        rating: 4,
        tags: ['popular', 'chef choice', 'steak']
    },
    {
        name: "burito",
        price: 14,
    ...
    },
    {
        ...
        popularity: 4,
        rating: 5,
        tags: ['dessert', 'ice cream', 'frozen', 'sweet']
    },
];
...
const filteredFood = foodArr.filter((currFood) => {
    for (const currTag of currFood['tags']) {
        if (currTag === 'popular') {
            return true;
        }
    }
...
function filterByProperty (property, number) {
    const filteredArr = foodArr.filter((currObj) => {
        for (const currKey in currObj) {
            if (currKey === property && currObj[currKey] > number) {
                return true;
            }
        }
        return false;
...
console.log("************************");
console.log(filterByProperty('price', 10));